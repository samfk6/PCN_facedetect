...
public:
    ...
    void LoadModel(const std::string model1,const std::string model2,const std::string model3,const std::string modelTrack,
            const std::string net1, const std::string net2, const std::string net3, const std::string netTrack);
    ...
    std::vector<Window2> Track(cv::Mat img,
            ncnn::Net &net_track, float thres, int dim, std::vector<Window2> &winList);
    ncnn::Net net_1,net_2,net_3,net_track;
    int minFace_;
    float scale_;
    int stride_;
    ...
    float augScale_ = 0.15;
    cv::Scalar mean_;
    const float mean_vals[3] = {104.0, 117.0, 123.0};
};
PCN::PCN(const std::string model1,const std::string model2,const std::string model3,const std::string modelTrack,
		const std::string net1, const std::string net2, const std::string net3, const std::string netTrack) : impl_(new Impl()){
    Impl *p = (Impl *)impl_;
    p->LoadModel(model1, model2, model3, modelTrack, net1, net2, net3, netTrack);
}
...
std::vector<Window> PCN::DetectTrack(cv::Mat img)
{
    ...
    std::vector<Window2> winList = p->Track(imgPad, p->net_track, p->trackThreshold_, 72, preList);
...
void Impl::LoadModel(const std::string model1,const std::string model2,const std::string model3,const std::string modelTrack,
		const std::string net1, const std::string net2, const std::string net3, const std::string netTrack){
    ...
    net_track.load_param(netTrack.c_str());
    net_track.load_model(modelTrack.c_str());
}
...
std::vector<Window2> Impl::Detect(cv::Mat img, cv::Mat imgPad)
{
    ...
    std::vector<Window2> winList = Stage1(img, imgPad, net_1, classThreshold_[0]);
    winList = NMS(winList, true, nmsThreshold_[0]);

    winList = Stage2(imgPad, img180, net_2, classThreshold_[1], 24, winList);
    winList = NMS(winList, true, nmsThreshold_[1]);

    winList = Stage3(imgPad, img180, img90, imgNeg90, net_3, classThreshold_[2], 48, winList);
...
std::vector<Window2> Impl::Track(cv::Mat img, ncnn::Net &net_track, float thres, int dim, std::vector<Window2> &winList)
{
    if (winList.size() == 0)
        return winList;
    std::vector<cv::Mat> dataList;
    for (uint i = 0; i < winList.size(); i++)
    {
        ...
        dataList.push_back(CropFace(img, win, dim));
    }


    std::vector<Window2> ret;
    for(uint ind=0; ind< winList.size(); ind++){
        ncnn::Extractor ex_track = net_track.create_extractor();
        ex_track.set_light_mode(true);
        ex_track.set_num_threads(threads);
        ncnn::Mat ncnn_img = ncnn::Mat::from_pixels_resize(dataList[ind].data, ncnn::Mat::PIXEL_RGB, dataList[ind].cols, dataList[ind].rows,dim,dim);
        ncnn_img.substract_mean_normalize(mean_vals, 0);
        ex_track.input("data", ncnn_img);
        ncnn::Mat reg,prob,rotateProb;
        ex_track.extract("bbox_reg", reg);
        ex_track.extract("rotate_reg", rotateProb);
        ex_track.extract("cls_prob", prob);
        ...
        if (prob[1] > thres){
            ...
            float xn = reg[1];
            float yn = reg[2];
            int cropX = winList[ind].x - augScale_ * winList[ind].w;
            int cropY = winList[ind].y - augScale_ * winList[ind].w;
            int cropW = winList[ind].w + 2 * augScale_ * winList[ind].w;

            float theta = -winList[ind].angle * M_PI / 180;
            ...
            if (Legal(x, y, img) && Legal(x + w - 1, y + w - 1, img))
            {
                ...
                if (tmpW >= 20)
                    ret.push_back(Window2(x + augScale_ * tmpW, y + augScale_ * tmpW,
                                          tmpW, tmpW, winList[ind].angle + angle, winList[ind].scale, prob[1]));
            }
        }
    }
    return ret;
...
int main()
{
    PCN detector("model/PCN-1.bin", "model/PCN-2.bin", "model/PCN-3.bin", "model/PCN-track.bin",
    		"model/PCN-1.proto", "model/PCN-2.proto", "model/PCN-3.proto", "model/PCN-track.proto");
    ...
    /// tracking
    detector.SetTrackingPeriod(200);
    ...
    while (1)
    {
		...
		tmp++;
		tm.stop();
		float gettime = tm.getTimeMilli();
		tm.start();

		if((gettime- begin) / 1000 > 1)
		{
			fps = (float)tmp/((gettime- begin)/1000);
